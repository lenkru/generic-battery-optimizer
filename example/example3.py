"""
Example 3: Optimize a household energy system with a heat pump and a battery.
"""
...
# Heat Pump with manufacturer COP data
# The optimizer will fit hplib COP parameters (p1-p4) from the manufacturer data
# This creates a more accurate model tailored to the specific heat pump
heat_pump = HeatPump(
    name="hp_bosch_compress7000_28",
    type="Generic",  # Use Generic to allow custom parameters
    id=1,  # Air/Water regulated (base parameters before customization)
    t_in=280.15,  # Reference source temperature: 7°C
    t_out=328.15,  # Reference sink temperature: 55°C (flow temperature)
    p_th=28000,  # Thermal power at reference: 28 kW
    ...
    cop_data=cop_data_manufacturer,  # Use manufacturer COP data to fit custom parameters
)
